...
export async function compileContract(): Promise<void> {
  await new Promise<void>((resolve, reject) =>
    ...
    child.exec(
      path.join(
        __dirname,
        "../ligo/exec_ligo compile contract " +
          path.join(__dirname, "../ligo/src/fa2_multi_asset.mligo") +
          " -e multi_asset_main -p hangzhou"
      ),
      (err, stdout) => {
        if (err) {
          ...
          console.log(
            "  " + path.join(__dirname, "../ligo/compiled/fa2_multi_asset.tz")
          );
          fs.writeFileSync(
            path.join(__dirname, "../ligo/compiled/fa2_multi_asset.tz"),
            stdout
...
export async function deployContract(): Promise<void> {
  ...
  if (!process.env.ORIGINATOR_PRIVATE_KEY || !process.env.ADMIN_ADDRESS) {
    throw new Error("Unable to start script, no keys");
  }
  ...
  try {
    ...
    toolkit.setProvider({
      signer: await InMemorySigner.fromSecretKey(
        process.env.ORIGINATOR_PRIVATE_KEY
      ),
    });
