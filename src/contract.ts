...
export async function compileContract(): Promise<void> {
    ...
    await new Promise<void>((resolve, reject) =>
        ...
        child.exec(
            path.join(__dirname, "../ligo/exec_ligo compile contract " + path.join(__dirname,  "../ligo/src/fa2_multi_asset.mligo") + " -e multi_asset_main -p hangzhou"),
            (err, stdout) => {
                if (err) {
...
export async function deployContract(): Promise<void> {
    ...
    try {
        ...
        toolkit.setProvider({ signer: await InMemorySigner.fromSecretKey('') });
